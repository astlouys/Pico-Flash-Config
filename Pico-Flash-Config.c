...
/* $TITLE=flash_erase() */
/* ============================================================================================================================================================= *\
         Erase configuration data in Pico's flash memory. The way the electronics is done, one sector of the flash (4096 bytes) must be erased at a time.
\* ============================================================================================================================================================= */
...
/* ============================================================================================================================================================= *\
                                 ...
                                 NOTE: It is assumed that the CRC16 is the last 2 bytes (16 bits) of the packet passed as an argument.
\* ============================================================================================================================================================= */
...
UINT8 flash_read_config(UINT32 DataOffset, UINT8 *Data, UINT16 DataSize)
{
  ...
  {
    uart_send(__LINE__, __func__, " =======================================================================================================================\r");
    uart_send(__LINE__, __func__, "      Entering flash_read_config()\r");
    uart_send(__LINE__, __func__, "      Read current configuration from Pico's flash\r");
    ...
    uart_send(__LINE__, __func__, "      Display configuration data retrieved from flash memory:\r");
    uart_send(__LINE__, __func__, " =======================================================================================================================\r");
  }
...
UINT8 flash_save_config(UINT32 DataOffset, UINT8 *Data, UINT16 DataSize)
{
  ...
  if (FlagLocalDebug)
  {
    uart_send(__LINE__, __func__, "Entering flash_save_config()\r");
    uart_send(__LINE__, __func__, "=========================================================================================================\r");
    uart_send(__LINE__, __func__, "     SAVING current configuration data to flash.\r");
    uart_send(__LINE__, __func__, "     XIP_BASE (flash base address):   0x%8.8X\r",           XIP_BASE);
    ...
    uart_send(__LINE__, __func__, "     Current configuration before computing CRC16:\r");
    util_display_data(Data, DataSize);
    ***/
  }

  /* Validate size of configuration data. */
  if (DataSize > 0x1000)
  {
    printf("\r\r\r\r\r");
    ...
    uart_send(__LINE__, __func__, "                       Configuration size to save to flash is too big (0x%4.4X)\r", DataSize);
  ...
  *(UINT16 *)(Data + DataSize - 2) = Crc16;
  ...
  if (FlagLocalDebug)
  {
    uart_send(__LINE__, __func__, " Display flash configuration as saved:\r");
    util_display_data(Data, DataSize);
  }

  if (FlagLocalDebug) uart_send(__LINE__, __func__, " Exiting flash_save_config())\r");
...
UINT8 flash_write(UINT32 DataOffset, UINT8 *NewData, UINT16 NewDataSize)
{
  ...
  /* A wear leveling algorithm has not been implemented since the flash usage for configuration data doesn't require it.
